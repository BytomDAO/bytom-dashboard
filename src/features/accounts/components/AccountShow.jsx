...
class AccountShow extends BaseShow {
  ...
  listAddress() {
    this.props
      .listAddress(this.props.params.id)
      .then((data) => {
        if (data.status !== 'success') {
          return;
        }
        const normalAddresses = data.data
          .filter((address) => !address.change)
          .map((address) => {
            return { address: address.address, program: address.controlProgram };
          });
        const changeAddresses = data.data
          .filter((address) => address.change)
          .map((address) => {
            return { address: address.address, program: address.controlProgram };
          });
  ...
  createAddress() {
    ...
    const ok = confirm(t('account.createConfirm'));
    if (ok) {
      this.props
        .createAddress({
          account_alias: this.props.item.alias,
              ...
              <KeyValueTable
                ...
                actions={
                  index === 0 && [
                    ...
                    <Link key="reset-password-btn" className="btn btn-link" to={`/accounts/key/reset-password/${key}`}>
                      {t('key.resetPassword')}
                    </Link>,
                  ]
                }
...
export default withNamespaces('translations')(connect(mapStateToProps, mapDispatchToProps)(AccountShow));
