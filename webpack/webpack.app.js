  ...
  html: function (context) {
    return {
      'index.html': context.defaultTemplate({
        ...
        head:
          process.env.NODE_ENV !== 'production' ? '<script data-dll="true" src="/dependencies.dll.js"></script>' : '',
      }),
    };
  },
...
for (let item of loaders) {
  ...
  if (item.loader && item.loader.indexOf('css-loader') > 0) {
    item.loader = item.loader.replace(
      'css-loader',
      'css-loader?module&importLoaders=1&localIdentName=[name]__[local]__[hash:base64:5]',
    );
  }
